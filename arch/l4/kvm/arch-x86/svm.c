...
#if defined(CONFIG_X86_64) || defined(CONFIG_X86_PAE) || defined(CONFIG_L4)
...
#ifdef CONFIG_L4
	if (!npt_enabled) {
		printk(KERN_ERR "kvm: must use Nested Paging on L4\n");
		r = -EOPNOTSUPP;
		goto err;
	}
#endif
...
{
	//l4/unsigned long host_cr4_mce = read_cr4() & X86_CR4_MCE;
...
static int handle_exit(struct kvm_vcpu *vcpu)
{
	...
	if (l4x_kvm_dbg()) {
		printk("%s(%d): e=%x ei1=%llx ei2=%llx rip=%llx\n", 
		       __func__, __LINE__,
